...
def _HMA_calc(time_heel, time_toe, heel_accel, toe_accel, HMA, h):
    ...
    HMA.toe_accel_horiz = np.sqrt(toe_accel.iloc[:, 0]**2 +
                                  toe_accel.iloc[:, 1]**2)
    # HMA.toe_accel_horiz = toe_accel.iloc[:, 1]     # y acceleration (y-axis is length of GS walkway)
    HMA.toe_accel_horiz = _butterworth_filter(HMA.toe_accel_horiz, HMA.time_toe, format='np', dim=1)

    # HMA HS events
    HMA.heel_accel_Z_peaks = _peaks_HMA(np.array(HMA.heel_accel_Z), format='np', ord=30)
    ...
    HMA.heel_jerk_Z_peaks = _peaks_HMA(np.array(HMA.heel_jerk_Z), format='np', ord=10)
